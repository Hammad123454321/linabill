...
use App\Models\NewBanner;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class NewBannerController extends Controller
{
    public function load_new_banner(Request $request)
    {
        ...
        foreach ($feature_selection as $data) {
            ...
            if ($data->image == "" || $data->image == null) {
                ...
                $image = '<img src="uploads/new_arrive_banner/'.$data->image.'" height="50" width="50" style="border-radius:10%"/>';
            }
    ...
    public function insert_new_banner(Request $request)
    {  
            ...
            if($request->hasFile('image')){
                ...
                $image->move(public_path('/uploads/new_arrive_banner/'), $new_image_name);
    ...
    public function edit_new_banner($id) 
    {
        $Data = DB::table('new_banners')
        ->select()
        ->where('id', $id)
        ->first();
    ...
    public function update_new_banner(Request $request)
    {
        
            $data             = NewBanner::find($request->input('featured_id'));
            $data->title      = $request->input('title');
            $data->desc       = $request->input('desc');
            $data->link       = $request->input('link');
            ...
            if($request->hasFile('up_image')){
                ...
                $oldImagePath = public_path('/uploads/new_arrive_banner/') . $data->image;
    ...
    public function delete_new_banner($id)
    {
        $data  = NewBanner::find($id);
        ...
        DB::table('new_banners')->where('id', $id)->delete();
